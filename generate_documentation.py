...
def parse_filter_target(expression_dict: Optional[Dict[str, Any]]) -> Optional[FilterTarget]:
    # This function is primarily used during the initial parsing step,
    # but we keep it here in case direct parsing from raw data is needed elsewhere.
    # The loading step now relies on the structure already saved in JSON.
    ...
    try:
        ...
        if "Column" in expression_dict:
            ...
            if source_ref is None: source_ref = column_dict.get("SourceRef")
            entity = None; property_name = column_dict.get("Property")
            if source_ref: entity = source_ref.get("Entity") or source_ref.get("Source")
            if entity and property_name: return FilterTarget(entity=entity, property=property_name)
        # Case 2: Aggregation Filter
        elif "Aggregation" in expression_dict:
            agg_dict = expression_dict["Aggregation"]; agg_func_map = {0: "Sum", 1: "Avg", 2: "Min", 3: "Max", 4: "Count", 5: "CountNonNull", 6: "Median", 7: "StandardDeviation", 8: "Variance"}
            agg_func_code = agg_dict.get("Function", -1); agg_func_name = agg_func_map.get(agg_func_code, f"Agg{agg_func_code}")
            ...
            if inner_expr and "Column" in inner_expr:
                 ...
                 entity = None; property_name = column_dict.get("Property")
                 if source_ref: entity = source_ref.get("Entity") or source_ref.get("Source")
                 if entity and property_name: return FilterTarget(entity=entity, property=f"{agg_func_name}({property_name})")
                 elif property_name: return FilterTarget(entity="Unknown", property=f"{agg_func_name}({property_name})")
        ...
        # Case 3: Measure Filter
             ...
             if source_ref: entity = source_ref.get("Entity") or source_ref.get("Source")
             if entity and property_name: return FilterTarget(entity=entity, property=f"[{property_name}]")
             elif property_name: return FilterTarget(entity="Measure", property=f"[{property_name}]")
    except Exception as e: print(f"Warning: Exception during filter target parsing: {e} - Data: {expression_dict}")
    return None
...
def load_parsed_data(parsed_dir: Path) -> Tuple[List[Relationship], List[Table], Dict[str, Any], List[ReportPage], List[ReportFilter]]:
    ...
    try: # Relationship loading
        ...
        if raw_rels: relationships = [Relationship(**rel_dict) for rel_dict in raw_rels]
        print(f"Loaded {len(relationships)} relationships from {rel_file_path.name}")
    except Exception as e: print(f"Error loading relationships: {e}")
    ...
    except Exception as e: print(f"Error loading model summary: {e}")
    tables_dir_path = parsed_dir / config.OUTPUT_TABLES_JSON_SUBDIR
    if tables_dir_path.is_dir(): # Table loading loop
        ...
        for table_file in tables_dir_path.glob("*.json"):
            try:
                ...
                for col_dict in raw_columns:
                    raw_annotations = col_dict.pop('annotations', []); col_annotations = [Annotation(**ann_dict) for ann_dict in raw_annotations]
                ...
                for mea_dict in raw_measures:
                     raw_annotations = mea_dict.pop('annotations', []); mea_annotations = [Annotation(**ann_dict) for ann_dict in raw_annotations]
            ...
            except Exception as e: print(f"Error loading/processing table {table_file.name}: {e}"); traceback.print_exc()
        print(f"Loaded data for {len(tables)} tables.")
    else: print(f"Warning: Tables directory not found: {tables_dir_path}")
    ...
    try:
        ...
        if report_layout_data:
            ...
            for i, filter_dict in enumerate(raw_report_filters):
                ...
                except Exception as filter_e: print(f"  ERROR deserializing a report-level filter: {filter_e} - Data: {filter_dict}"); traceback.print_exc()
            ...
            for page_dict_orig in raw_pages:
                ...
                try:
                    ...
                    for j, filter_dict in enumerate(raw_page_filters):
                         ...
                         except Exception as filter_e: print(f"    ERROR deserializing a page-level filter on page '{page_name_for_debug}': {filter_e}")
                    ...
                    for visual_dict_orig in raw_visuals:
                        ...
                        try:
                            ...
                            for fm_dict in raw_field_mappings:
                                try: fm_dict.setdefault('role', 'Unknown'); field_mapping_objects.append(VisualFieldMapping(**fm_dict))
                                except Exception as fm_e: print(f"      ERROR processing a field mapping in visual '{visual_name_for_debug}': {fm_e}")
                        ...
                        except Exception as visual_e: print(f"    ERROR processing visual '{visual_name_for_debug}' on page '{page_name_for_debug}': {visual_e}"); traceback.print_exc()
                    # Create Page Object
                    page_obj = ReportPage(**page_dict, page_level_filters=page_filter_objects, visuals=visual_objects)
                    report_pages.append(page_obj)
                except Exception as page_e: print(f"ERROR processing page dictionary for page '{page_name_for_debug}': {page_e}"); traceback.print_exc()
            print(f"\nFinished processing pages. Created {len(report_pages)} page objects.")
    except Exception as e: print(f"FATAL Error processing report layout file {report_layout_file_path.name}: {e}"); traceback.print_exc()
...
def initialize_llm() -> Optional[AzureChatOpenAI]:
    ...
    if not all([config.AZURE_OPENAI_ENDPOINT, config.AZURE_OPENAI_API_KEY, config.AZURE_OPENAI_API_VERSION, config.AZURE_OPENAI_CHAT_DEPLOYMENT_NAME]):
    ...
    try:
        ...
        return llm
    except Exception as e: print(f"Error initializing AzureChatOpenAI: {e}"); traceback.print_exc(); return None

# --- Function to Load Prompt Template from File ---
def load_prompt_template(template_path: Path) -> str:
    # (Implementation uses config)
    try:
        if not template_path.is_file(): raise FileNotFoundError(f"Prompt template file not found: {template_path}")
        return template_path.read_text(encoding='utf-8')
    except Exception as e: print(f"ERROR: Failed to load prompt template from {template_path}: {e}"); return "Error: Could not load prompt template. Input: {input}"
...
except Exception as e: print(f"CRITICAL ERROR during prompt template loading: {e}. Exiting."); raise e

# --- Async Overview Generation Function ---
async def generate_model_overview(llm: AzureChatOpenAI, tables: List[Table], relationships: List[Relationship]) -> str:
    # (Implementation uses loaded template)
    print("\n--- Generating Model Overview ---")
    overview = "(Overview generation failed)"
    if not llm: return "(LLM not available for overview)"
    output_parser = StrOutputParser()
    if "Error: Could not load" in MODEL_OVERVIEW_TEMPLATE_STR: return "(Overview generation skipped: Error loading prompt template)"
    overview_chain: RunnableSequence = MODEL_OVERVIEW_TEMPLATE | llm | output_parser
    if not tables: return "(Cannot generate overview: No table information available)"
    try:
        table_names = sorted([t.name for t in tables if not t.isHidden])
        table_list_str = ", ".join([f"'{name}'" for name in table_names])
        rel_summaries = []
        for i, rel in enumerate(relationships):
            from_table_visible = any(t.name == rel.fromTable and not t.isHidden for t in tables); to_table_visible = any(t.name == rel.toTable and not t.isHidden for t in tables)
            if rel.isActive and from_table_visible and to_table_visible:
                if len(rel_summaries) >= 10: rel_summaries.append("..."); break
    ...
    except Exception as e: print(f"  Error generating model overview: {e}"); overview = f"(Error generating overview: {e})"
    return overview
...
async def enrich_data_with_llm(llm: AzureChatOpenAI, tables: List[Table]):
    ...
    if not llm: print("  LLM not available, skipping enrichment."); return
    ...
    for i, table in enumerate(tables):
        ...
        visible_cols = [col for col in table.columns if not col.isHidden]; example_cols = [f"{col.name} ({col.dataType})" for col in visible_cols[:10]]; example_cols_str = ", ".join(example_cols) if example_cols else "N/A"
        all_tasks.append(_enrich_item_with_semaphore(table_chain, {"table_name": table.name, "column_examples": example_cols_str}, table, "llm_description"))
        # Schedule Column/Measure Tasks
        for column in visible_cols:
            all_tasks.append(_enrich_item_with_semaphore(column_chain, {"column_name": column.name, "data_type": column.dataType, "table_name": table.name, "table_description": lambda t=table: t.llm_description or "N/A"}, column, "llm_description"))
            if isinstance(column, CalculatedColumn) and column.daxExpression: all_tasks.append(_enrich_item_with_semaphore(dax_chain, {"item_name": f"Calculated Column: {column.name}", "dax_code": column.daxExpression}, column, "llm_dax_explanation"))
        visible_measures = [m for m in table.measures if not m.isHidden]
        for measure in visible_measures:
            if measure.daxExpression: all_tasks.append(_enrich_item_with_semaphore(dax_chain, {"item_name": f"Measure: {measure.name}", "dax_code": measure.daxExpression}, measure, "llm_dax_explanation"))
            else: measure.llm_dax_explanation = "(No DAX expression found)"
    # Run all collected tasks concurrently
    if all_tasks: print(f"\nRunning {len(all_tasks)} enrichment tasks concurrently across all tables (Limit: {config.LLM_CONCURRENCY_LIMIT})..."); await asyncio.gather(*all_tasks); print(f"Finished all concurrent enrichment tasks.")
    else: print("No enrichment tasks to run.")
...
async def enrich_filters_with_llm(llm: AzureChatOpenAI, report_filters: List[ReportFilter], pages: List[ReportPage]):
    """
    Enriches ReportFilter objects with LLM explanations for complex definitions.
    Modifies filter objects in the provided lists directly.
    """
    print("\n--- Enriching Filters with LLM Explanations (Async + Semaphore) ---")
    if not llm: print("  LLM not available, skipping filter enrichment."); return
    ...
    async def _enrich_filter_with_semaphore(filt: ReportFilter):
        ...
        try:
            filter_def_str = json.dumps(filt.filter_definition, indent=2)
             # Limit length sent to LLM if needed
            # MAX_FILTER_DEF_LEN = 1000
            # if len(filter_def_str) > MAX_FILTER_DEF_LEN:
            #     filter_def_str = filter_def_str[:MAX_FILTER_DEF_LEN] + "\n... (truncated)"
        except Exception:
...
def assemble_report(
    ...
    """Assembles the final Markdown documentation file including report structure."""
            ...
            try:
                ...
                for clause in where_clauses:
                    ...
                    if comparison and isinstance(comparison, dict):
                        ...
                        if "Column" in left_obj: left_expr_str = left_obj.get("Column", {}).get("Property", "N/A")
                        elif "Measure" in left_obj: measure_prop = left_obj.get('Measure', {}).get('Property', 'N/A'); left_expr_str = f"[{measure_prop}]"
                        elif "Aggregation" in left_obj:
                             agg_dict = left_obj["Aggregation"]; agg_func_map = {0: "Sum", 1: "Avg", 2: "Min", 3: "Max", 4: "Count", 5: "CountNonNull"}
                             agg_func_code = agg_dict.get("Function", -1); agg_func_name = agg_func_map.get(agg_func_code, f"Agg{agg_func_code}")
                             inner_prop = agg_dict.get("Expression",{}).get("Column",{}).get("Property", "?"); left_expr_str = f"{agg_func_name}({inner_prop})"
                        ...
                        op_map = {0: "=", 1: "<>", 2: ">", 3: ">=", 4: "<", 5: "<="}; op = op_map.get(comp_kind, f"Kind({comp_kind})")
                        condition_summary = f"`{left_expr_str}` {op} {right_val_str}"

                    elif in_cond and isinstance(in_cond, dict):
                        left_expr_obj = in_cond.get("Expressions", [{}])[0]; left_expr_str = "N/A"
                        if "Column" in left_expr_obj: left_expr_str = left_expr_obj.get("Column", {}).get("Property", "N/A")
                        elif "Measure" in left_expr_obj: measure_prop = left_expr_obj.get('Measure', {}).get('Property', 'N/A'); left_expr_str = f"[{measure_prop}]"
                        values = in_cond.get("Values", [[]])[0]; value_str = ", ".join([f"`{v.get('Literal',{}).get('Value', '?')}`" for v in values[:5]])
                        if len(values) > 5: value_str += ", ..."; condition_summary = f"`{left_expr_str}` IN ({value_str})"

                    if is_negated and condition_summary != "(Unparsed condition)": condition_summary = f"NOT ({condition_summary})"
                    conditions.append(condition_summary)

                if conditions: definition_summary = " AND ".join(conditions)
                else: definition_summary = "(Could not parse conditions)"

            except Exception as e:
                print(f"WARNING: Error parsing filter definition for filter '{filt.name}': {e}"); filter_def_str_raw = json.dumps(filt.filter_definition)
                if len(filter_def_str_raw) > 100: filter_def_str_raw = filter_def_str_raw[:100] + "..."; definition_summary = f"(Raw JSON: `{filter_def_str_raw}`)"
        elif filt.filter_definition: definition_summary = f"(Non-dict definition: `{str(filt.filter_definition)[:100]}`)"
    ...
    def format_mapping(mapping: VisualFieldMapping) -> str:
         ...
         display = mapping.display_name or mapping.query_ref or "N/A"; role_str = str(mapping.role) if mapping.role else 'N/A'
    ...
    try: # Main report writing block
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            if relationships:
                 active_rels = [r for r in relationships if r.isActive]; inactive_rels = [r for r in relationships if not r.isActive]; sorted_rels = sorted(active_rels, key=lambda r: (r.fromTable.lower(), r.fromColumn.lower())) + sorted(inactive_rels, key=lambda r: (r.fromTable.lower(), r.fromColumn.lower()))
                 if sorted_rels:
                     f.write("The following relationships link the tables:\n\n")
                     for rel in sorted_rels: from_table_hidden = any(t.name == rel.fromTable and t.isHidden for t in tables); to_table_hidden = any(t.name == rel.toTable and t.isHidden for t in tables); hidden_note = " (Involves hidden table)" if from_table_hidden or to_table_hidden else ""; details = []; [details.append("**Inactive**") for _ in [1] if not rel.isActive]; [details.append(f"Filter: {rel.crossFilteringBehavior}") for _ in [1] if rel.crossFilteringBehavior != 'singleDirection']; details_str = f" ({', '.join(details)})" if details else ""; f.write(f"* `{rel.fromTable}`.[`{rel.fromColumn}`] -> `{rel.toTable}`.[`{rel.toColumn}`]{details_str}{hidden_note}\n")
                     f.write("\n")
            ...
            if sorted_visible_tables:
                 for table in sorted_visible_tables:
                     table_slug = slugify(f"table-{table.name}"); f.write(f"#### <a name=\"{table_slug}\"></a>Table: `{table.name}`\n\n"); desc = table.llm_description or table.description or "_No description available._"; f.write(f"{desc}\n\n")

                     # --- Columns (Corrected Loop) ---
                     visible_columns = sorted([col for col in table.columns if not col.isHidden and not isinstance(col, CalculatedColumn)], key=lambda c: c.name.lower())
                     if visible_columns:
                         f.write("##### Columns\n\n")
                     ...
                     # Calculated Columns...
                     visible_calc_columns = sorted([col for col in table.columns if not col.isHidden and isinstance(col, CalculatedColumn)], key=lambda c: c.name.lower())
                     if visible_calc_columns: f.write("##### Calculated Columns\n\n"); [ (f.write(f"**`{col.name}`** (`{col.dataType}`)\n\n"), [f.write(f"- **Description:** {col_desc}\n") for col_desc in [col.llm_description or col.description] if col_desc], [f.write(f"- **DAX Expression:**\n```dax\n{col.daxExpression.strip()}\n```\n") for _ in [1] if col.daxExpression], [f.write(f"- **DAX Explanation (Generated):** {dax_expl}\n") for dax_expl in [col.llm_dax_explanation] if dax_expl], f.write("\n") ) for col in visible_calc_columns]
                     # Measures...
                     visible_measures = sorted([m for m in table.measures if not m.isHidden], key=lambda m: m.name.lower())
                     if visible_measures: f.write("##### Measures\n\n"); [ (f.write(f"**`{measure.name}`**\n\n"), [f.write(f"- **Description (Generated):** {mea_desc}\n") for mea_desc in [measure.llm_description] if mea_desc], [f.write(f"- **DAX Expression:**\n```dax\n{measure.daxExpression.strip()}\n```\n") for _ in [1] if measure.daxExpression], [f.write(f"- **DAX Explanation (Generated):** {dax_expl}\n") for dax_expl in [measure.llm_dax_explanation] if dax_expl], f.write("\n") ) for measure in visible_measures]
                     f.write("---\n\n")
            ...
            if report_level_filters: f.write("### <a name=\"report-level-filters\"></a>Report Level Filters\n\n"); f.write("The following filters are applied to the entire report:\n\n"); [f.write(f"- {format_filter(filt)}\n") for filt in report_level_filters]; f.write("\n")
            else: f.write("_No report-level filters found._\n\n")
            # Report Pages...
            if report_pages:
                 f.write("### <a name=\"report-pages\"></a>Report Pages\n\n"); sorted_pages_body = sorted(report_pages, key=lambda p: (p.display_name or p.name or "").lower())
                 for page in sorted_pages_body:
                     page_title = page.display_name or page.name or "Unnamed Page"; page_slug = slugify(f"page-{page_title}"); f.write(f"#### <a name=\"{page_slug}\"></a>Page: {page_title}\n\n"); f.write(f"*Internal Name: `{page.name}`, Ordinal: {page.ordinal}*\n\n")
                     # Page Filters...
                     if page.page_level_filters: f.write("##### Page Level Filters\n\n"); [f.write(f"- {format_filter(filt)}\n") for filt in page.page_level_filters]; f.write("\n")
                     # Visuals...
                     if page.visuals:
                         f.write("##### Visuals on this Page\n\n"); sorted_visuals = sorted(page.visuals, key=lambda v: (v.y or 0, v.x or 0))
                         for visual in sorted_visuals:
                             visual_title = visual.title or visual.visual_type or visual.name or "Unnamed Visual"; f.write(f"**{visual_title}**\n\n"); f.write(f"- Type: `{visual.visual_type}`\n"); f.write(f"- Name: `{visual.name}`\n")
                             if visual.llm_description: f.write(f"- Description (Generated): {visual.llm_description}\n")
                             if visual.field_mappings: f.write("- Fields Used:\n"); [f.write(f"  - {format_mapping(mapping)}\n") for mapping in visual.field_mappings]
                             else: f.write("- Fields Used: _(None detected)_\n")
                             if visual.visual_level_filters: f.write("- Visual Level Filters:\n"); [f.write(f"  - {format_filter(filt)}\n") for filt in visual.visual_level_filters]
                             f.write("\n")
                     else: f.write("_No visuals found on this page._\n\n"); f.write("\n---\n\n")
            else: f.write("_No page details were loaded or parsed._\n\n")
...
def write_full_json_output(output_path: Path, overview: str, relationships: List[Relationship], tables: List[Table], report_pages: List[ReportPage], report_level_filters: List[ReportFilter]):
    # (Implementation uses config and DataClassEncoder)
    print(f"\n--- Writing full data JSON output to: {output_path} ---")
    full_data = {"model_overview": overview, "relationships": relationships, "tables": tables, "report_level_filters": report_level_filters, "pages": report_pages}
    try:
        output_path.parent.mkdir(parents=True, exist_ok=True)
        with open(output_path, 'w', encoding='utf-8') as f: json.dump(full_data, f, cls=DataClassEncoder, indent=2, ensure_ascii=False)
        print(f"Successfully wrote full data JSON to {output_path}")
    except TypeError as e: print(f"ERROR writing full data JSON file {output_path}: TypeError - {e}"); print("This might indicate an issue with serializing a specific data type."); traceback.print_exc()
    except Exception as e: print(f"ERROR writing full data JSON file {output_path}: {e}"); traceback.print_exc()
...
async def run_generation():
    ...
    parsed_dir = config.OUTPUT_DIR; output_md_file = config.OUTPUT_DIR / config.OUTPUT_MARKDOWN_FILE; output_json_file = config.OUTPUT_DIR / config.OUTPUT_FULL_DATA_JSON_FILE
    generate_md = 'md' in config.OUTPUT_FORMATS; generate_json = 'json' in config.OUTPUT_FORMATS
    if not generate_md and not generate_json: print("WARNING: No output formats selected in config.OUTPUT_FORMATS. Skipping generation."); return
    print("\nStep G1: Loading Parsed JSON Data..."); relationships, tables, model_summary, report_pages, report_level_filters = load_parsed_data(parsed_dir)
    if not tables and not report_pages: print("Error: No table or report page data loaded. Exiting generation step."); return
    print("\nStep G2: Initializing LLM..."); llm = initialize_llm()
    print("\nStep G3: Generating Model Overview..."); overview_text = await generate_model_overview(llm, tables, relationships)
    print("\nStep G4: Enriching Model Data..."); await enrich_data_with_llm(llm, tables)
    print("\nStep G4.5: Enriching Filter Definitions...")
    await enrich_filters_with_llm(llm, report_level_filters, report_pages)
    if generate_md: print("\nStep G5: Assembling Final Markdown Report..."); assemble_report(output_path=output_md_file, overview=overview_text, relationships=relationships, tables=tables, report_pages=report_pages, report_level_filters=report_level_filters)
    else: print("\nStep G5: Skipping Markdown Report generation (not in config.OUTPUT_FORMATS).")
    if generate_json: print("\nStep G6: Writing Full Data JSON Output..."); write_full_json_output(output_path=output_json_file, overview=overview_text, relationships=relationships, tables=tables, report_pages=report_pages, report_level_filters=report_level_filters)
    else: print("\nStep G6: Skipping Full Data JSON generation (not in config.OUTPUT_FORMATS).")
    end_time = time.time(); print(f"\n--- Documentation Generation Step Complete ---"); print(f"Generation Time: {end_time - start_time:.2f} seconds")
    if generate_md: print(f"Markdown Output: {output_md_file}")
    if generate_json: print(f"JSON Output:     {output_json_file}")
